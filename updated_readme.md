# SSH server setup, web server installation, and provides you with all connection details
...
- Configures SSH on port 8022 (configurable)
- Generates host keys automatically
- Provides exact SSH command to use

### 🌐 **Web Server Setup**
- Installs Apache with PHP support
- Creates web root directory
- Sets up virtual host on port 8080
- Creates beautiful dashboard with server info

### ⚡ **24/7 Persistence**
- Auto-starts on device boot (with Termux:Boot)
- Watchdog monitoring (auto-restarts if crashed)
- Keep-alive service prevents sleep
- Management script for easy control

### 📊 **Management Features**
- `server` command to control everything
- Web-based dashboard with status
- File manager for easy file uploads
...
- Enter a password or let it auto-generate
- Wait 2-3 minutes for installation
- Script shows you **EVERYTHING** you need

### **4. Connect from your PC**
```bash
# Use the EXACT command shown:
ssh -p 8022 username@192.168.x.x
# Enter the password shown
```

## **WHAT YOU GET:**

✅ **Fully automated** - No manual configuration needed  
✅ **SSH access** - Remote command line from any PC  
✅ **Web server** - Host websites from your Android  
✅ **File manager** - Upload/download files via browser  
✅ **24/7 operation** - Never stops running  
✅ **Auto-recovery** - Self-healing if crashes  
✅ **Dashboard** - Monitor everything from browser  
✅ **Credentials file** - All info saved to text file  

## **CONNECTION METHODS:**

### **From PC:**
```bash
ssh -p 8022 username@your-android-ip
```

### **From Browser:**
```
http://your-android-ip:8080
```
...
1. **Your IP address** (multiple interfaces)
2. **SSH command** (copy-paste ready)
3. **Web URL** (for browser access)
4. **Username & password**
